#!/bin/bash
# FlatBuffers iOS Simulator (Intel x86_64) 构建脚本
# 需要: macOS with Xcode and iOS Simulator SDK

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BUILD_DIR="${SCRIPT_DIR}/build_ios_sim_x86_64"
INSTALL_DIR="${SCRIPT_DIR}/../Cpp/flatbuffers_ios_sim_x86_64"

echo "=== Building FlatBuffers for iOS Simulator (Intel x86_64) ==="
...
# 运行 CMake 配置 (Intel Simulator)
cmake -DCMAKE_OSX_ARCHITECTURES=x86_64 \
      -DCMAKE_OSX_SYSROOT="${IOS_SIM_SDK}" \
      -DCMAKE_SYSTEM_NAME=iOS \
      -DCMAKE_SYSTEM_VERSION=13.0 \
      -DCMAKE_C_FLAGS="-target x86_64-apple-ios13.0-simulator" \
      -DCMAKE_CXX_FLAGS="-target x86_64-apple-ios13.0-simulator -std=c++17" \
